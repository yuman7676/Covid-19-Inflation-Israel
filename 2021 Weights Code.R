...
#title: "COVID-19 Inflation 2021 Weights"
...
##monthly averges 
#General
cc.monthly1a <- cc.long.1 %>%
  filter(year(date)==2020) %>%
  group_by(month(date),ResearchG) %>%
  summarize(average=mean(value))
#Detailed
cc.monthly2a <- cc.long.2 %>%
  filter(year(date)==2020) %>%
...
change.rate1 <- function(data, x, m) {
  (cc.monthly1a$average[cc.monthly1a$ResearchG==x & cc.monthly1a$`month(date)`==m]
   -(cc.monthly1a$average[cc.monthly1a$ResearchG==x & cc.monthly1a$`month(date)`==1]
     +cc.monthly1a$average[cc.monthly1a$ResearchG==x & cc.monthly1a$`month(date)`==2])/2)/
    (cc.monthly1a$average[cc.monthly1a$ResearchG==x & cc.monthly1a$`month(date)`==1]
     +cc.monthly1a$average[cc.monthly1a$ResearchG==x & cc.monthly1a$`month(date)`==2])
}
change.rate2 <- function(x,m) {
  (cc.monthly2a$average[cc.monthly2a$DetailG==x & cc.monthly2a$`month(date)`==m]
   -(cc.monthly2a$average[cc.monthly2a$DetailG==x & cc.monthly2a$`month(date)`==1]
     +cc.monthly2a$average[cc.monthly2a$DetailG==x & cc.monthly2a$`month(date)`==2])/2)/
    (cc.monthly2a$average[cc.monthly2a$DetailG==x & cc.monthly2a$`month(date)`==1]
     +cc.monthly2a$average[cc.monthly2a$DetailG==x & cc.monthly2a$`month(date)`==2])
}

cc.monthly1a <- cc.monthly1a %>%
  mutate(change_rate=change.rate1(x=ResearchG, m=`month(date)`)) 

cc.monthly2a <- cc.monthly2a %>%
...
Wt_Furniture <- c(36.47,36.47,36.47,36.47)
